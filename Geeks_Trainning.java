...
class Solution {
    public int maximumPoints(int arr[][]) {
        // code here
        int storage[][]= new int[arr.length][4];
        for (int i = 0; i < arr.length; i++) {
           Arrays.fill(storage[i], -1);
        }
        int prevtask=3;
        return trainMemo(arr,storage,arr.length-1,prevtask);
    }
    public int trainMemo(int[][]arr,int storage[][],int index,int prevtask){
        if(index<0){
            return 0;
        }
         if(index==0){
             ...
             for(int j=0;j<3;j++){
                 if(j!=prevtask){
                  int maxCount=arr[index][j];
                  maxi=Math.max(maxi,maxCount);
                 }
             }
             return maxi;
         }
         if(storage[index][prevtask]!=-1){
             return storage[index][prevtask];
         }
         int max=Integer.MIN_VALUE;
         for(int i=0;i<3;i++){
             
             if(i!=prevtask){
                 int points=arr[index][i]+trainMemo(arr,storage,index-1,i);
                 max=Math.max(max,points);
             }
         }
         return storage[index][prevtask]=max;
    }
}
